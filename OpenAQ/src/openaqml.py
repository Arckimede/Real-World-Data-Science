...
for name, model in models.items():
  ...
  results[name] = {'MAE: ': mae, 'R² score: ': r2Score}
  print(f'{name}: MAE: {mae} | R²: {r2Score}')
...
# plotting metrics (MAE and R² score)
maes, r2Scores = [], [],
...
For all three models, most residuals are very close to zero but there are a few extreme points that are far from zero, creating huge spikes. This indicates highly skewed target values; some values are very small (a few µg/m³) and some are extremely large.

Most predictions are clustered near the origin but the extreme values dominate the error calculation.
